...
class GroqClient:
    ...
    def __init__(
        ...
        # Test connection (optional - log errors but don't fail initialization)
    ...
    def _test_connection(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Successfully connected to Groq API with model: {self.model}")
        except Exception as e:
            # Log the error but don't fail initialization
            self.logger.warning(f"Could not test connection to Groq API: {e}")
            self.logger.info("API connection will be tested on first actual request")
    ...
    def _check_rate_limits(self, estimated_tokens: int) -> None:
        ...
        self._token_usage = [
            (t, tokens) for t, tokens in self._token_usage if current_time - t < 60
        ]
        ...
        if current_token_usage + estimated_tokens > self.tokens_per_minute:
            ...
            oldest_usage_time = (
                min(t for t, _ in self._token_usage) if self._token_usage else current_time
            )
            sleep_time = 60 - (current_time - oldest_usage_time)
            if sleep_time > 0:
                self.logger.warning(
                    f"Token limit would be exceeded, sleeping for {sleep_time:.2f} seconds"
                )
                time.sleep(sleep_time)
    ...
    def analyze_code(
        self, code: str, context: str = "", analysis_type: str = "security", **kwargs: Any
    ) -> LLMResponse:
